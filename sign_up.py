...
class Sign_Up(QMainWindow):
    ...
    def initUI(self):
        ...
        self.id = QtWidgets.QLabel("Enter login ")
        self.password = QtWidgets.QLabel("Enter password(min 4)  ")
        ...
        self.NextBtn.clicked.connect(lambda: self.btn_click(self.NextBtn))
        self.NextBtn.setMaximumWidth(120)
        ...
        self.btn_layout.addWidget(self.NextBtn)
        ...
        self.main_layout.addWidget(self.btn_widget, 5, 4)
    ...
    def btn_click(self,b):
        if b.text()=='Next':
            ...
            if not line is None:
                    self.message = QtWidgets.QLabel("This login already exists.")
                    self.main_layout.addWidget(self.message,4,3,1,4)
            else:
                    if len(pswd)>=4:
                        self.message = QtWidgets.QLabel("Success")
                        self.main_layout.addWidget(self.message,4,3,1,4)
                        self.dialog = Example(id,pswd,self.selected)
                        self.dialog.show()
                        #self.close()
                    else:
                        self.message = QtWidgets.QLabel("Password has to be more than 3 symbols.")
                        self.main_layout.addWidget(self.message,4,3,1,4)
            #self.message.setAlignment(Qt.AlignCenter)
            self.message.setFixedSize(400,20)
